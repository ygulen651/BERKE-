...
export function AddPaymentDialog({ employees, shoots, transactions }: AddPaymentDialogProps) {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        // Her seçilen personel için ayrı bir işlem kaydı oluştur (Muhasebe takibi için daha sağlıklı)
        const amountPerPerson = Number(values.amount) / values.relatedIds.length
        
        const promises = values.relatedIds.map(staffId => {
            const employee = employees.find(e => e.id === staffId)
            return createTransaction({
                ...values,
                amount: amountPerPerson,
                relatedId: staffId,
                relatedName: employee?.name || "Bilinmeyen",
                type: "EXPENSE",
        ...
        if (results.every(r => r.success)) {
            ...
            alert("Bazı ödemeler kaydedilirken hata oluştu.")
        }
    }
